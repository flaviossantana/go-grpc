...
type UserServiceClient interface {
	...
	AddUsersStream(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUsersStreamClient, error)
}
...
func (c *userServiceClient) AddUsersStream(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUsersStreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[2], "/pb.UserService/AddUsersStream", opts...)
	if err != nil {
		return nil, err
	}
	x := &userServiceAddUsersStreamClient{stream}
	return x, nil
}

type UserService_AddUsersStreamClient interface {
	Send(*User) error
	Recv() (*UserResultStream, error)
	grpc.ClientStream
}

type userServiceAddUsersStreamClient struct {
	grpc.ClientStream
}

func (x *userServiceAddUsersStreamClient) Send(m *User) error {
	return x.ClientStream.SendMsg(m)
}

func (x *userServiceAddUsersStreamClient) Recv() (*UserResultStream, error) {
	m := new(UserResultStream)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type UserServiceServer interface {
	...
	AddUsersStream(UserService_AddUsersStreamServer) error
...
func (UnimplementedUserServiceServer) AddUsersStream(UserService_AddUsersStreamServer) error {
	return status.Errorf(codes.Unimplemented, "method AddUsersStream not implemented")
}
...
func _UserService_AddUsersStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UserServiceServer).AddUsersStream(&userServiceAddUsersStreamServer{stream})
}

type UserService_AddUsersStreamServer interface {
	Send(*UserResultStream) error
	Recv() (*User, error)
	grpc.ServerStream
}

type userServiceAddUsersStreamServer struct {
	grpc.ServerStream
}

func (x *userServiceAddUsersStreamServer) Send(m *UserResultStream) error {
	return x.ServerStream.SendMsg(m)
}

func (x *userServiceAddUsersStreamServer) Recv() (*User, error) {
	m := new(User)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var UserService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "AddUsersStream",
			Handler:       _UserService_AddUsersStream_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
